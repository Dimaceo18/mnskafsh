...
# ── Месяцы для обратного перевода ────────────────────────────────────────────
MONTHS_RU_FULL: dict[str, str] = {
...
# ═══════════════════════════════════════════════════════════════════════════════
#  НОВЫЕ ФУНКЦИИ ДЛЯ СОВМЕСТИМОСТИ С CHANNEL_BOT.PY
# ═══════════════════════════════════════════════════════════════════════════════

# Алиас для обратной совместимости с channel_bot.py
normalize_venue = normalize_place


def parse_event_date(date_raw: str) -> tuple[str, datetime | None]:
    """
    Парсит дату из строки, возвращает (форматированная_строка, datetime_объект)
    """
    if not date_raw:
        return "Дата не указана", None
    
    # Сначала пробуем ISO формат (из bezkassira)
    if 'T' in date_raw:
        try:
            dt = datetime.fromisoformat(date_raw.replace('Z', '+00:00'))
            formatted = dt.strftime('%d %B %Y, %H:%M')
            # Переводим месяц на русский
            for en, ru in MONTHS_RU_FULL.items():
                formatted = formatted.replace(en, ru)
            return formatted, dt
        except:
            pass
    
    # Пробуем текстовый парсинг
    date_iso = parse_text_date(date_raw)
    time_str = extract_time(date_raw)
        ...
        try:
            ...
            if time_str:
                try:
                    ...
                    pass
            formatted = dt.strftime('%d %B %Y, %H:%M') if time_str else dt.strftime('%d %B %Y')
            # Переводим месяц на русский
            for en, ru in MONTHS_RU_FULL.items():
                formatted = formatted.replace(en, ru)
            return formatted, dt
...
def detect_category(title: str) -> str:
    ...
    categories = {
        'concert': ['концерт', 'live', 'фестиваль', 'выступление', 'группа', 'каверы'],
        'theater': ['театр', 'спектакль', 'опера', 'балет', 'трагедия', 'комедия'],
        'cinema': ['кино', 'фильм', 'премьера фильма', 'кинопоказ', 'синема'],
        'exhibition': ['выставка', 'экспозиция', 'вернисаж', 'галерея'],
        'kids': ['детям', 'детский', 'сказка', 'кукольный', 'ёлка', 'ребенок'],
        'sport': ['спорт', 'футбол', 'хоккей', 'матч', 'соревнование'],
        'free': ['бесплатно', 'free'],
    }
...
def normalize_event(event: dict) -> dict | None:
    """
    Основная функция нормализации события.
    Принимает сырое событие из парсера, возвращает нормализованное.
    """
    try:
        ...
        # Получаем место
        place = event.get('venue', event.get('place', ''))
        known_venues = event.get('_known_venues', None)
        venue = normalize_place(place, known_venues)
        
        # Проверка на Минск (если не Минск — пропускаем)
        if not is_minsk_event(venue):
            logger.debug(f"Пропущено событие не из Минска: {title} ({venue})")
            return None
        
        # Нормализация даты
        date_raw = event.get('date', event.get('event_date', ''))
        date_normalized, date_dt = parse_event_date(date_raw)
        
        # Нормализация цены
        from_free = event.get('_from_free_section', False)
        price = normalize_price(event.get('price', ''), from_free)
        
        # Если цена не определена, но событие из free-секции
        if not price and from_free:
            price = "Бесплатно"
        
        # Возрастное ограничение
        age = '0+'
        age_match = re.search(r'(\d+)\+', event.get('description', '') + ' ' + title)
        if age_match:
            age = f"{age_match.group(1)}+"
        
        # Категория
        category = event.get('category', detect_category(title))
        if price == "Бесплатно" and category == 'all':
            category = 'free'
        
        return {
            'id': event.get('id', f"{title}_{date_normalized}_{venue}"),
            'title': title,
            'venue': venue,
            ...
            'age': age,
            'category': category,
            'image': event.get('image', event.get('img', '')),
        ...
        logger.error(f"Ошибка нормализации события: {e}")
        return None


def normalize_events(events: list) -> list:
    """Нормализует список событий"""
    normalized = []
    for event in events:
